...
public class singletonImprimanteServiceCSV {
    ...
    static ro.arthursplaytime.gui.tehnologii.singletonImprimanteServiceCSV singletonImprimanteServiceCSV = null;

    public static ro.arthursplaytime.gui.tehnologii.singletonImprimanteServiceCSV getInstance(){
    ...
    private imprimante getFromCSV(String line){
        ...
        for (int i = 1; i < values.length - 3; i = i + 3) {
            ...
            ro.arthursplaytime.gui.tehnologii.filament fil = new filament(values[i],Integer.parseInt(values[i+1]),values[i+2]);
            filament.add(fil);
        }
